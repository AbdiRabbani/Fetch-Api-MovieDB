...
const url_main = "https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=true&language=en-US&page=1&sort_by=popularity.desc"
const url_detail = "https://api.themoviedb.org/3/movie/";
const url_img = "http://image.tmdb.org/t/p/w500"


// Home
fetch(url_main, options)
  .then(data => data.json())
  .then(data => {
    film = data.results;
    film.forEach(row => {
...
document.getElementById('img-thumbnaill').setAttribute('src', '/assets/images/smoking cat.png')
...
fetch(url_detail + movieId, options)
  .then(data => data.json())
  .then(data => {
    console.log(data);
